...
func handleConflict(d db.DB, rule ngmodels.AlertRule, logger log.Logger, query alertRule, conflictErr error) error {
	...
	err = d.WithDbSession(context.Background(), func(sess *db.Session) error {
		exists, err = sess.Cols("uid, title, namespace_uid").Get(&conflictingRule)
		return err
	})

	if err != nil {
		logger.Error("Detected rule conflict but failed to fetch conflicting rule", "rule", rule, "error", err)
		return ngmodels.ErrAlertRuleConflict(rule, errors.New("detected rule conflict but failed to fetch conflicting rule"))
	}
	...
	return ngmodels.ErrAlertRuleConflictVerbose(
		ngmodels.AlertRule{
			UID:          conflictingRule.UID,
			Title:        conflictingRule.Title,
			NamespaceUID: conflictingRule.NamespaceUID,
		},
		rule,
		conflictErr,
	)
